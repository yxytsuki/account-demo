...
export default {
  ...
  methods: {
    // 月份时间戳
    GetTimeStamp () {
      const startDate = Date.parse(this.year) // 每年1月1日时间戳
      const MonthStart = [] // 存储月份时间戳

      for (let i = 0; i < 12; i++) {
        const MonStart = new Date(startDate).setMonth(new Date(startDate).getMonth() + i)
    ...
    getPeriod () {
      // 开始年份时间戳
      const startDate = Date.parse(this.year)
      // 结束年份时间戳
